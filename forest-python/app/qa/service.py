...
class QaService:
    ...
    async def ask(self, user_id: int, group_id: int, question: str,
                  session_id: Optional[int] = None, rerank: bool | None = None) -> dict:
        ...
        bundle = await retrieval.retrieve(group_id, question, planned_queries, rerank=rerank)
...
请基于证据回答问题。如果证据不足，请说明。"""
    ...
    async def ask_stream(self, user_id: int, group_id: int,
                         question: str, session_id: Optional[int] = None, rerank: bool | None = None) -> AsyncIterator[dict]:
        start_time = time.perf_counter()
