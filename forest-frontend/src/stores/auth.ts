...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    clearSession() {
      ...
      localStorage.removeItem('forest_access_token')
      localStorage.removeItem('forest_current_user_id')
    },
    setSession(accessToken: string, currentUser: CurrentUserProfile, refreshToken: string | null = null) {
      ...
      localStorage.setItem('forest_access_token', accessToken)
      localStorage.setItem('forest_current_user_id', String(currentUser.userId))
      // Save to account switcher list
      try {
        const accounts = JSON.parse(localStorage.getItem('forest_accounts') ?? '[]') as Array<{
          userId: number
          displayName: string
          systemRole: string
          accessToken: string
        ...
        localStorage.setItem('forest_accounts', JSON.stringify(filtered.slice(0, 5)))
      } catch { /* ignore */ }
    },
    async runBootstrap(): Promise<CurrentUserProfile | null> {
      ...
      if (!this.accessToken) {
        const saved = localStorage.getItem('forest_access_token')
      ...
      try {
        if (this.accessToken !== null) {
          try {
            ...
            // Token expired, try cookie refresh
          }
        }
        ...
        const expectedUserId = localStorage.getItem('forest_current_user_id')
