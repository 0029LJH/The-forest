...
function getSavedAccounts(): SavedAccount[] {
  try {
    return JSON.parse(localStorage.getItem('argus_accounts') ?? '[]') as SavedAccount[]
  } catch { return [] }
}
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-brand" @click="router.push('/')">
        ...
        <div v-show="!sidebarCollapsed" class="sidebar-brand__text">
          <span class="sidebar-brand__eyebrow">Forest · Platform</span>
          <span class="sidebar-brand__title">深林知识库</span>
        </div>
      </div>
      ...
      <header class="topbar">
        <div class="topbar-left">
          ...
          <div class="breadcrumb">
            <span class="breadcrumb-eyebrow">Forest</span>
            <span class="breadcrumb-sep">/</span>
            <span class="breadcrumb-current">{{ route.meta.title || 'Forest 知识库' }}</span>
          </div>
        </div>
        <div class="topbar-right">
          <div class="user-menu-wrapper">
            ...
            <div v-if="showUserMenu" class="user-dropdown">
              ...
              <!-- Saved accounts -->
              <template v-if="savedAccounts.length > 0">
                <div class="dropdown-divider"></div>
                <div class="dropdown-section-label">切换账号</div>
                <button
                  v-for="acc in savedAccounts"
                  :key="acc.userId"
                  class="dropdown-item dropdown-item--account"
                  :class="{ 'is-current': acc.userId === authStore.currentUser?.userId }"
...
.dropdown-item--account.is-current {
  background: rgba(74, 144, 217, 0.06);
}
